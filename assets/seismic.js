/* ===========================================================================
   ...
   Shared across the teaching module sets; this copy ships with
...
const SEIS = (function () {
  ...
  /* Hilbert transform of a single trace, via the analytic signal: zero the
     ...
     the result is the quadrature. It is what phaseRotate below is built on. */
  function hilbert(x) {
    const n0 = x.length;
    let n = 1;
    while (n < 2 * n0) n *= 2;
  ...
  /* A short label on a colored ground.

     The box is kept inside the canvas. A left-aligned tag placed against the
     right-hand panel of a row used to run off the edge and be cut in half,
     which happens more the narrower the column gets, so it is nudged back
     rather than left where it was asked for. Clamping a label a few pixels is
     always better than losing its last four words. */
  function tag(ctx, x, y, text, color, align) {
    ctx.save();
    ctx.font = '10px "IBM Plex Mono", monospace';
    const w = ctx.measureText(text).width + 8;
    let ax = align === 'right' ? x - w : x;
    const cw = (ctx.canvas && ctx.canvas.style && parseFloat(ctx.canvas.style.width)) || 0;
    const ch = (ctx.canvas && ctx.canvas.style && parseFloat(ctx.canvas.style.height)) || 0;
    let ay = y;
    if (cw) {
      if (ax + w > cw - 2) ax = cw - 2 - w;
      if (ax < 2) ax = 2;
    }
    if (ch) {
      if (ay + 7 > ch - 1) ay = ch - 8;
      if (ay - 7 < 1) ay = 8;
    }
    ctx.fillStyle = color;
    ctx.fillRect(ax, ay - 7, w, 14);
    ctx.fillStyle = '#fff';
    ctx.textAlign = 'left'; ctx.textBaseline = 'middle';
    ctx.fillText(text, ax + 4, ay + 0.5);
    ctx.restore();
  }
  ...
  /* ---------------------------------------------------------------------
     POP THE EXERCISES OUT

     The exercises and the key points live in tab panes, so reading them means
     leaving the panel a student is supposed to be working. This opens them in
     a second window, styled with the same stylesheet, so the two can sit side
     by side.
  ...
  function popOutTeaching(nodes, title) {
    ...
    let w;
    try {
      w = window.open('', '', 'width=640,height=860,scrollbars=yes,resizable=yes');
    } catch (e) { return false; }
    if (!w) return false;
    // The panes are hidden tab panels in the parent, so the copies have their
    // hidden attribute stripped and their tab roles dropped.
    ...
    w.document.write(
      ...
      '.teach{display:block;padding:0}.tryit,.takeaway{margin-bottom:20px;max-width:none}' +
      '.tabpane{display:block}</style>' +
  ...
  // Every module carries a #popBtn inside its exercises pane. The panes it
  // opens are the exercises and the key points, which are the two a student
  // wants beside the controls. Wiring it here keeps it out of every module.
  if (typeof document !== 'undefined' && document.addEventListener) {
    document.addEventListener('DOMContentLoaded', function () {
      ...
      btn.addEventListener('click', function () {
        ...
        const panes = [document.getElementById('pe'), document.getElementById('pk')];
        const ok = popOutTeaching(panes, document.title);
        if (note) {
          note.textContent = ok ? '' : 'Your browser blocked the window — the exercises are on this page.';
        }
      });
    });
  }
  ...
  /* ---------------------------------------------------------------------
     GUIDES ACROSS PANELS

     Two panels side by side are only comparable if the eye can carry a depth
     from one to the other. This draws faint rules at the same two-way times
     across every panel in a row, labels them with depth at the far left, and
     shades the named beds so a student can see where the coal actually is
     rather than being told about it in prose.

       rects  the panels in the row, left to right, all sharing one y mapping
       yFn    (t, rect) -> y, the module's own time-to-y function
       ticks  [{ t, label, right }]      faint rules, labeled depth at the
                                        left and two-way time at the right
       beds   [{ t0, t1, label, color }] shaded bands, labeled at the left

     A student meeting this material has to learn that the well is measured in
     depth and the seismic is measured in time, and that the two are not
     proportional to each other. Carrying both scales on the same rules is the
     cheapest way to make that visible: the depth spacing is even and the time
     spacing is not.
     --------------------------------------------------------------------- */

  function guides(ctx, rects, yFn, ticks, beds) {
    if (!rects || !rects.length) return;
    const first = rects[0], last = rects[rects.length - 1];
    const x0 = first.x, x1 = last.x + last.w;

    (beds || []).forEach((b) => {
      const ya = yFn(b.t0, first), yb = yFn(b.t1, first);
      const top = Math.min(ya, yb), h = Math.max(2, Math.abs(yb - ya));
      if (top + h < first.y || top > first.y + first.h) return;
      ctx.save();
      ctx.fillStyle = b.color || 'rgba(132,22,23,.13)';
      rects.forEach((r) => {
        const t = Math.max(r.y, top), bt = Math.min(r.y + r.h, top + h);
        if (bt > t) ctx.fillRect(r.x, t, r.w, bt - t);
      });
      ctx.restore();
      if (b.label) {
        ctx.save();
        ctx.font = '10px "IBM Plex Mono", monospace';
        ctx.fillStyle = b.text || '#841617';
        ctx.textAlign = 'right'; ctx.textBaseline = 'middle';
        ctx.fillText(b.label, x0 - 8, top + h / 2);
        ctx.restore();
      }
    });

    (ticks || []).forEach((t) => {
      const y = yFn(t.t, first);
      if (y < first.y - 0.5 || y > first.y + first.h + 0.5) return;
      dashedLine(ctx, x0, y, x1, y, 'rgba(92,102,112,.30)', [2, 4], 1);
      if (t.label || t.right) {
        ctx.save();
        ctx.font = '10px "IBM Plex Mono", monospace';
        ctx.fillStyle = '#9AA1A8';
        ctx.textBaseline = 'middle';
        if (t.label) { ctx.textAlign = 'right'; ctx.fillText(t.label, x0 - 8, y); }
        if (t.right) { ctx.textAlign = 'left'; ctx.fillText(t.right, x1 + 8, y); }
        ctx.restore();
      }
    });
  }
  ...
  /* ---------------------------------------------------------------------
     ...
     A student working through a module should not have to go back up to the
     tab strip to reach the next step. This appends a previous/next row to the
     foot of every step pane, labeled from the tabs themselves, and clicks the
     matching tab so each module's own wiring does the work.

     A module that builds its own navigation is left alone.
     --------------------------------------------------------------------- */
  ...
  function buildStepNav() {
    ...
    const label = (id) => {
      const b = tabs.find((t) => t.dataset.tab === id);
      return b ? b.textContent.replace(/^\s*\d+\s*·\s*/, '').trim() : id;
    };
    /* Which panes count as numbered steps, and in what order.

       This used to be `ids.filter(/^p\d+$/)`, which assumed a module's steps
       were called p1, p2, p3 in that order. Module 8 names two of its panes
       pmap and pps and does not run its p-numbered panes in numeric order, so
       the counter there read "step 1 of 3" on what the tab strip called step 3.
       The tab strip is the thing the student is reading, so the numbering comes
       from it: a tab counts as a step if it is outside the meta group at the
       end, and the count follows the order the tabs are in. */
    const numbered = tabs
      .filter((b) => !b.closest('.tabs-meta'))
      .map((b) => b.dataset.tab);
    ...
    ids.forEach((id, i) => {
      ...
      const mk = (target, text, ghost) => {
        ...
        b.addEventListener('click', () => {
          const t = document.querySelector('#tabs button[data-tab="' + target + '"]');
          if (t) t.click();
          const hint = document.querySelector('.tabhint');
          if (hint && hint.scrollIntoView) hint.scrollIntoView({ behavior: 'smooth', block: 'start' });
        });
        return b;
      };

      nav.appendChild(prev ? mk(prev, '\u2190 ' + label(prev), true)
                           : document.createElement('span'));
      const where = document.createElement('span');
      where.className = 'where';
      const k = numbered.indexOf(id);
      where.textContent = k >= 0 ? 'step ' + (k + 1) + ' of ' + numbered.length : label(id);
      nav.appendChild(where);
      nav.appendChild(next ? mk(next, label(next) + ' \u2192')
                           : document.createElement('span'));
  ...
  return {
    ...
    UNITS, readState, writeState, copyLink, savePNG, popOutTeaching, buildStepNav, guides,
  };
})();
